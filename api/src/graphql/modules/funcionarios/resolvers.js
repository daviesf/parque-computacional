...
export default {
    Query: {
        ...
        funcionario: async (_, { idFuncionario }) => {
            const funcionario = await knex('funcionarios').where('idFuncionario', idFuncionario).first();
            return funcionario;
        },
    ...
    Mutation: {
        createFuncionario: async (_, { data }) => {
            ...
            const novoFuncionario = await knex('funcionarios').where('idFuncionario',id).first();
            return novoFuncionario;
        },
        
        updateFuncionario: async (_,{ idFuncionario, data}) => {
            const funcionarioAtualizado = await knex('funcionarios').where('idFuncionario', idFuncionario).update(data);
            const funcionario = await knex('funcionarios').where('idFuncionario', idFuncionario).first();
            return funcionario;
        },
