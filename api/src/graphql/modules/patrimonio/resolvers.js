...
export default {
    Query: {
        ...
        patrimonio: async (_, { idPatrimonio }) => {
            const patrimonio = await knex("patrimonio").where("idPatrimonio", idPatrimonio).first();
            return patrimonio;
        },
        ...
        patrimoniosByBancada: async (_, { idBancada }) => {
            const patrimonios = await knex("patrimonio").where("idBancada", idBancada);
            return patrimonios;
        },
        patrimoniosByMarcaModelo: async (_, { marca, modelo }) => {
            const patrimonios = await knex("patrimonio").where("marca", marca).orWhere("modelo", modelo);
            return patrimonios;
        },
    ...
    Mutation: {
        createPatrimonio: async (_, { data }) => {
            ...
            const novoPatrimonio = await knex("patrimonio").where("idPatrimonio", id).first();
            return novoPatrimonio;
        },
        updatePatrimonio: async (_, { idPatrimonio, data }) => {
            const patrimonioAtualizado = await knex("patrimonio").where("idPatrimonio", idPatrimonio).update(data);
            const patrimonio = await knex("patrimonio").where("idPatrimonio", idPatrimonio).first();
            return patrimonio;
        },
